...
let flippedCards = [];
let attempts = 30;    // Tentativas restantes
let timeLeft = 60;   // Tempo restante
let score = 0;        // Pontuação do jogador
let completedRounds = 0; // Contador de vezes que todos os pares foram completados
let consecutivePairs = 0; // Número de pares consecutivos acertados

// Exibindo as cartas no tabuleiro
function renderDeck(deck) {
    board.innerHTML = ''; // Limpa a grid de cartas
    deck.forEach(card => {
...
function checkWin() {
    ...
    if (remainingCards.length === 0) {
        completedRounds++; // Incrementa o contador de rodadas completadas

        if (completedRounds === 3) {
            clearInterval(timer); // Para o temporizador
        score += 1000; // Bônus de vitória
        calculateBonus(); // Calcula bônus de tempo e tentativas
        scoreElement.textContent = score; // Atualiza a pontuação final
            endGame(true); // Finaliza o jogo com vitória
        } else {
            dropCards(); // Animação de "caindo" das cartas
            setTimeout(() => {
                addMoreCards(); // Adiciona 6 novos pares
            }, 2000);
        }
    }
}

// Função para adicionar novos 6 pares ao deck e redesenhar o tabuleiro
function addMoreCards() {
    ...
    const updatedDeck = [...newCards, ...newCards]; // Apenas os novos pares
    deck = updatedDeck.sort(() => 0.5 - Math.random()); // Embaralha os novos pares
    renderDeck(deck); // Renderiza apenas os novos pares no tabuleiro
}

// Função para animar as cartas caindo
function dropCards() {
    const allCards = document.querySelectorAll('.card');
    allCards.forEach(card => {
        card.classList.add('fall');  // Aplicando uma classe de queda
    });
}   
